...
# 2-pass
# parser.add_option("-d", "--d", default=512, help="1st layer embedding dimension")
# parser.add_option("-l", "--l", default=256, help="2nd layer embedding dimension")
# parser.add_option("-k", "--k", default=256, help="The dimension of project metrices k")
...
dim_pred = int(opts.k)
dim_pass = int(opts.d)
# If 2-pass, uncomment the following 3 lines
# dim_pass_2 = int(opts.l)
...
class Model(object):
    ...
    def _build_model(self):
        ...
        # passing 2 times
        # W1 = weight_variable([dim_pass + dim_pass_2, dim_pass_2])
        # b1 = bias_variable([dim_pass_2])
        # tf.add_to_collection('l2_reg', tf.contrib.layers.l2_regularizer(1.0)(W1))

        # disease_vector2 = tf.nn.l2_normalize(tf.nn.relu(tf.matmul(
        #     tf.concat([tf.matmul(self.disease_semantic_normalize, a_layer(disease_vector1, dim_pass_2)) +
        #                tf.matmul(self.disease_symptom_normalize, a_layer(disease_vector1, dim_pass_2)) +
        #                tf.matmul(self.disease_interact_normalize, a_layer(disease_vector1, dim_pass_2)) +
        #                tf.matmul(self.disease_microbe_elevated_normalize, a_layer(microbe_vector1, dim_pass_2)) +
        #                tf.matmul(self.disease_microbe_reduced_normalize, a_layer(microbe_vector1, dim_pass_2)),
        #                disease_vector1], axis=1), W1) + b1), dim=1)
        #
        # microbe_vector2 = tf.nn.l2_normalize(tf.nn.relu(tf.matmul(
        #     tf.concat([tf.matmul(self.microbe_taxa_normalize, a_layer(microbe_vector1, dim_pass_2)) +
        #                tf.matmul(self.microbe_interact_normalize, a_layer(microbe_vector1, dim_pass_2)) +
        #                tf.matmul(self.microbe_disease_elevated_normalize, a_layer(disease_vector1, dim_pass_2)) +
        #                tf.matmul(self.microbe_disease_reduced_normalize, a_layer(disease_vector1, dim_pass_2)),
        #                microbe_vector1], axis=1), W1) + b1), dim=1)
        #
        # self.disease_representation = disease_vector2
        # self.microbe_representation = microbe_vector2
